...
\begin{document}
\begin{center}
{\Large Additive and Abelian Categories}\\[4pt]
Algebraic Topology I \textbullet\ Lecture 1
\end{center}
...
\end{definition}
A diagram commutes when composites along paths with the same endpoints agree.
...
Since $[f,0]=f\rho_1$, we have $f+0=f\rho_1\Delta_A=f$, and likewise $0+f=f$. The interchange of the two summands fixes $\Delta_A$, and composing $[f,g]$ with that interchange gives $[g,f]$; thus $f+g=g+f$.

For associativity, let $\Delta_A^{(3)}\colon A\to A\oplus A\oplus A$ have all three components $\id_A$. Both $(f+g)+h$ and $f+(g+h)$ equal
\[
[f,g,h]\Delta_A^{(3)}.
\]
This follows by comparing the three product components and the three coproduct restrictions.
...
\end{definition}
The structures agree because $[f,g]\Delta_A=(f\rho_1+g\rho_2)\Delta_A=f+g$.
...
\end{proof}
For modules, $\coim f=A/\ker f$, $\im f=f(A)\subseteq B$, and $\fbar([a])=f(a)$. 
